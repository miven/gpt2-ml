...
def fabu(siteid, title, content):
    url = 'http://118.25.148.148/hackworry/fabu.php'
    data = {'siteid': siteid, 'title': title,'content': content}
...
with tf.Session(config=tf_config, graph=tf.Graph()) as sess:
    ...
    while True:
        try:
            r = requests.get('http://118.25.148.148/hackworry/api.php').text
            print(r)
            json_str = json.loads(r)
            siteid = json_str['siteid']
            keyword = json_str['keywords']
            print("站点ID：", siteid)
            print("关键词: ", keyword)
